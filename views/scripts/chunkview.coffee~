...
calcPoint = (pos, opts) ->
  verts = []
  verts.push pos[0] + opts.chunkX * 16 * 1.00000 - 15*16.0
  verts.push (pos[1] + 1) * 1.0
  verts.push pos[2] + opts.chunkZ * 16 * 1.00000 - 15*16.0
  verts
...
class ChunkView
  constructor: (@options, @indices, @vertices) -> 
    ...
    #@databuf = new ArrayBuffer(16 * 16 * 256)
    #@data = new Int16Array(@databuf)    
  ...
  getBlockAt: (x, y, z) =>
    #bl = @data[x+z*16+y*256]
    #if not (bl is -20) then return bl
    ...
    for section in sections
      if section isnt undefined and section.Y * 1 is sectionnum * 1   
        #@data[x+z*16+y*256] = section.Blocks[blockpos]
  ...
  addPoint: (p) =>
    p3 = calcPoint(p, @options)
    @vertices.push p3
    block = @getBlockType p[0],p[1],p[2]
    @colors.push block.rgba
    return
